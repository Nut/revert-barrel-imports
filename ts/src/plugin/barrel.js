...
class BarrelFilesMapping {
  ...
  createSpecifiersMapping(fullPathModule, forceFullScan = false) {
    ...
    barrelAST.program.body.every((node) => {
      ...
      const convertedExportedPath = webpackConfig.convertAliasToOriginal(
        fullPathModule,
        originalExportedPath
      );
      let absoluteExportedPath = PathFunctions.getModuleAbsolutePath(
        fullPathModule,
        convertedExportedPath
      );
      if (t.isExportNamedDeclaration(node)) {
        node.specifiers.forEach((specifier) => {
          ...
          this.mapping[fullPathModule][specifierExportedName] =
            this.createDirectSpecifierObject(
        ...
        Object.assign(
          this.mapping[fullPathModule],
          this.mapping[absoluteExportedPath]
        );
        ...
        node.specifiers.forEach((specifier) => {
          ...
          const absoluteExportedPath = PathFunctions.getModuleAbsolutePath(
            fullPathModule,
            convertedExportedPath
          );
  ...
  createDirectSpecifierObject(
    ...
    if (this.isBarrelFile(fullPathModule)) {
      const originalPath =
        this.mapping[fullPathModule][specifierLocalName]["path"];
      const originalExportedName =
        this.mapping[fullPathModule][specifierLocalName]["exportedName"];
      const originalLocalName =
        this.mapping[fullPathModule][specifierLocalName]["localName"];
      const originalType =
        this.mapping[fullPathModule][specifierLocalName]["type"];
      return this.createDirectSpecifierObject(
        originalPath,
...
const importDeclarationVisitor = (path, state) => {
  ...
  if (AST.getSpecifierType(originalImportsSpecifiers[0]) === "namespace")
    return;
  const parsedJSFile = state.filename;
  const originalImportsPath = path.node.source.value;
  const convertedImportsPath = webpackConfig.convertAliasToOriginal(
    parsedJSFile,
    originalImportsPath
  );
  if (PathFunctions.checkIfModule(convertedImportsPath)) return;
  const importModuleAbsolutePath = PathFunctions.getModuleAbsolutePath(
    parsedJSFile,
    convertedImportsPath
  );
  if (!mapping.isBarrelFile(importModuleAbsolutePath)) return;
  const directSpecifierASTArray = originalImportsSpecifiers.map((specifier) => {
...
module.exports = { importDeclarationVisitor };
